...
std::uint8_t* Image::bits() { return image.data(); }

const std::uint8_t* Image::bits() const { return image.data(); }

void Image::fill(const Color& color) {
  std::array<std::uint8_t, 4> colorArray{color.red(), color.green(),
                                         color.blue(), color.alpha()};

  for (auto pixel = image.begin(); pixel != image.end();
       pixel = std::copy(colorArray.cbegin(),
                         std::next(colorArray.cbegin(), numChannels), pixel)) {
  }
}

int Image::height() const { return imageHeight; }
    ...
    for (auto col = 0; col < width() / 2; ++col) {
      for (auto channel = 0; channel < numChannels; ++channel) {
        std::swap(
            ...
            image[numChannels * ((row + 1) * width() - col - 1) + channel]);
      }
    }
  }
}
...
void Image::mirrorVertical() {
  for (auto row = 0; row < height() / 2; ++row) {
    for (auto col = 0; col < width(); ++col) {
      for (auto channel = 0; channel < numChannels; ++channel) {
        std::swap(image[numChannels * (row * width() + col) + channel],
...
int Image::width() const { return imageWidth; }
