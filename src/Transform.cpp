...
glm::vec3 Transform::getForward() const { return getLocalToWorldRotation()[0]; }

glm::vec3 Transform::getLeft() const { return getLocalToWorldRotation()[1]; }
...
glm::vec3 Transform::getUp() const { return getLocalToWorldRotation()[2]; }
...
glm::mat4 Transform::getWorldToLocalMatrix() const {
  ...
  return glm::lookAt(position, position + localToWorldRotation[0],
...
void Transform::translate(const glm::vec3& translate, Space relativeTo) {
  this->position +=
      relativeTo == Space::Self ? transformDirection(translate) : translate;
}
