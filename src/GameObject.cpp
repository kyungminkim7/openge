...
ge::Mesh::MeshData createCubeMeshData() {
  return {std::vector<ge::Mesh::Position>{
              // back face
          ...
          std::vector<ge::Mesh::Index>{// back face
                                       0u, 1u, 2u, 1u, 0u, 3u,
                                       ...
                                       // right face
                                       12u, 13u, 14u, 13u, 12u, 15u,

                                       // bottom face
                                       16u, 17u, 18u, 18u, 19u, 16u,

                                       // top face
                                       20u, 21u, 22u, 21u, 20u, 23u},
...
std::shared_ptr<ge::Material> createPrimitiveMaterial() {
  auto shaderProgram = ge::RenderPipeline::getShaderProgram(Program::Standard);
...
unsigned int getNumIndices(const aiMesh* mesh) {
  return std::accumulate(
      mesh->mFaces, mesh->mFaces + mesh->mNumFaces, 0u,
      [](auto sum, const auto& face) { return sum + face.mNumIndices; });
}
...
ge::Mesh::MeshData createMeshData(const aiMesh* mesh) {
  ...
  // Load indices.
  for (auto face = mesh->mFaces; face < mesh->mFaces + mesh->mNumFaces;
       ++face) {
    data.indices.insert(data.indices.cend(), face->mIndices,
...
std::shared_ptr<ge::GLTexture> createTexture(const aiScene* scene,
  ...
  texture = std::shared_ptr<ge::GLTexture>(
      new ge::GLTexture(ge::Image(filepath.c_str())), textureDeleter);
...
std::shared_ptr<ge::Material> createMaterial(const aiScene* scene,
  ...
  auto diffuseTexture = createTexture(scene, mesh, dir, aiTextureType_DIFFUSE);

  auto specularTexture =
      createTexture(scene, mesh, dir, aiTextureType_SPECULAR);
...
void addNode(ge::GameObject* gameObject, const aiScene* scene,
             const aiNode* node, const std::filesystem::path& dir) {
  for (auto i = node->mMeshes; i < node->mMeshes + node->mNumMeshes; ++i) {
  ...
  for (auto child = node->mChildren;
       child < node->mChildren + node->mNumChildren; ++child) {
