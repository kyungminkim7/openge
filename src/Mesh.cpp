...
constexpr auto POSITION_STRIDE =
    ge::Mesh::Position::length() * sizeof(ge::Mesh::Position::value_type);

constexpr auto NORMAL_STRIDE =
    ge::Mesh::Normal::length() * sizeof(ge::Mesh::Normal::value_type);
...
Mesh::Mesh(std::shared_ptr<GameObject> gameObject, const MeshData& meshData,
...
void Mesh::render() {
  ...
  const auto model =
      glm::scale(transform->getLocalToWorldMatrix(), transform->getScale());
...
void Mesh::setVertexAttributes(std::size_t positionsOffset,
  ...
  // Enable attributes
  shaderProgram->enableAttributeArray(ge::RenderPipeline::Attribute::POSITION);

  shaderProgram->enableAttributeArray(ge::RenderPipeline::Attribute::NORMAL);

  shaderProgram->enableAttributeArray(
      ge::RenderPipeline::Attribute::TEXTURE_COORDINATE);

  // Set attributes
  shaderProgram->setAttributeBuffer(ge::RenderPipeline::Attribute::POSITION,
                                    GL_FLOAT, positionsOffset,
                                    Position::length(), POSITION_STRIDE);

  shaderProgram->setAttributeBuffer(ge::RenderPipeline::Attribute::NORMAL,
                                    GL_FLOAT, normalsOffset, Normal::length(),
                                    NORMAL_STRIDE);

  shaderProgram->setAttributeBuffer(
      ge::RenderPipeline::Attribute::TEXTURE_COORDINATE, GL_FLOAT,
      textureCoordinatesOffset, TextureCoordinate::length(),
...
void Mesh::uploadVertices(const MeshData& meshData,
  ...
  const auto textureCoordinatesSize =
      meshData.textureCoordinates.size() * TEXTURE_COORDINATE_STRIDE;

  const auto meshDataSize =
      positionsSize + normalsSize + textureCoordinatesSize;
  ...
  vertexBuffer.write(positionsOffset, meshData.positions.data(), positionsSize);

  vertexBuffer.write(normalsOffset, meshData.normals.data(), normalsSize);

  vertexBuffer.write(textureCoordinatesOffset,
                     meshData.textureCoordinates.data(),
                     textureCoordinatesSize);

  setVertexAttributes(positionsOffset, normalsOffset, textureCoordinatesOffset,
                      shaderProgram);
}
