...
GLShaderProgram::~GLShaderProgram() { glDeleteProgram(program); }
...
void GLShaderProgram::bind() { glUseProgram(program); }
...
void GLShaderProgram::link() {
  ...
  if (!success) {
    ...
    std::string logStr(log.cbegin(), std::next(log.cbegin(), length));
    throw BuildError(logStr);
  }
}

void GLShaderProgram::release() { glUseProgram(0); }

void GLShaderProgram::setAttributeBuffer(const char* name, GLenum type,
                                         int offset, int tupleSize,
                                         int stride) {
  glVertexAttribPointer(attributeLocation(name), tupleSize, type, GL_FALSE,
