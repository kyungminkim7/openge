...
Skybox::Skybox(const char* front, const char* back, const char* left,
               const char* right, const char* up, const char* down)
    : shaderProgram(
          RenderPipeline::getShaderProgram(RenderPipeline::Program::Skybox)),
      vertexArray(),
...
Skybox::~Skybox() { glDeleteTextures(1, &texture); }
...
void Skybox::uploadTexture(const char* front, const char* back,
  ...
  shaderProgram->setUniformValue(RenderPipeline::Uniform::SKYBOX_TEXTURE,
...
void Skybox::uploadVertexData() {
  constexpr auto positionStride =
      Position::length() * sizeof(Position::value_type);
  ...
  vertexBuffer.allocate(POSITIONS.data(), POSITIONS.size() * positionStride);
