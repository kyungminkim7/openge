...
template <typename Event, typename RegisterListener, typename T,
          typename Listener>
void addListener(RegisterListener registerListener, const T* key,
                 Listener listener) {
  [[maybe_unused]] static auto init = [registerListener =
                                           std::move(registerListener)]() {
