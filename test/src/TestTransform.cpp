...
TEST(TransformTranslate, TranslatesRelativeToPosition) {
    ...
    ASSERT_THAT(transform.getPosition(),
                VecNear(CoordinateSystem::FORWARD + CoordinateSystem::LEFT,
                        0.1));
}
...
TEST(TransformLookAt, RotatesTransformToPointToTarget) {
    ...
    const auto expectedForward =
        glm::normalize(target - transform.getPosition());

    transform.lookAt(target);

    ASSERT_THAT(transform.getForward(),
                VecNear(expectedForward, 0.1f));
}
