...
ImageDecoder::~ImageDecoder() { AImageDecoder_delete(decoder); }

void ImageDecoder::setAndroidBitmapFormat(std::int32_t format) {
  const auto status = AImageDecoder_setAndroidBitmapFormat(decoder, format);
  if (status != ANDROID_IMAGE_DECODER_SUCCESS) {
    throw ImageDecoderError(status,
...
void ImageDecoder::setUnpremultipliedRequired(bool required) {
  ...
  if (status != ANDROID_IMAGE_DECODER_SUCCESS) {
    throw ImageDecoderError(
        status, "Failed to set unpre-multiply alpha for "s + filepath);
  }
}
...
std::vector<std::uint8_t> ImageDecoder::decodeImage(std::size_t stride,
  ...
  const auto status =
      AImageDecoder_decodeImage(decoder, pixels.data(), stride, size);
  if (status != ANDROID_IMAGE_DECODER_SUCCESS) {
    throw ImageDecoderError(status, "Failed to decode image "s + filepath);
  }
