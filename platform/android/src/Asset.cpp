...
Asset::Asset(const char* filepath, Mode mode)
    : asset(
          AAssetManager_open(AssetManager::get(), filepath,
...
Asset::~Asset() { AAsset_close(asset); }

AAsset* Asset::get() { return asset; }

std::size_t Asset::getLength() const { return AAsset_getLength64(asset); }
